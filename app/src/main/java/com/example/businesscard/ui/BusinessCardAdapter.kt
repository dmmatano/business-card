...
class BusinessCardAdapter: ListAdapter<BusinessCard, BusinessCardAdapter.ViewHolder>(DiffCalback()) {
    ...
    inner class ViewHolder (private val binding: ItemBusinessCardBinding): RecyclerView.ViewHolder(binding.root){
        fun bind(item: BusinessCard){
            with(binding){
                ...
                mvcContent.setCardBackgroundColor(Color.parseColor(item.fundoPersonalizado))
                mvcContent.setOnClickListener {
                    listenerShare(it)
                }
            }
        }
